...
const DEFAULT_REQUEST_TIMEOUT_MILLISECONDS = 2500;
...
const proxyEvents$1 = ['close', 'error', 'headers', 'listening'];

class RPCServer extends EventEmitter {
  constructor(...args) {
    super();

    _defineProperty(this, "_onConnection", client => {
    ...
    _defineProperty(this, "onClientError", client => error => {
      this.emit('error', error, client, this);
    });

    _defineProperty(this, "onClientRequest", client => request => {
  ...
  on(event, handler) {
    if (proxyEvents$1.includes(event)) {
...
WsNodeRPCWebSocket.CONNECTING = ws.CONNECTING;
WsNodeRPCWebSocket.OPEN = ws.OPEN;
WsNodeRPCWebSocket.CLOSING = ws.CLOSING;
WsNodeRPCWebSocket.CLOSED = ws.CLOSED;
WsNodeRPCWebSocket.Server = RPCServer;

module.exports = WsNodeRPCWebSocket;
