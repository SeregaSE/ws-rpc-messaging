...
var NodeRPCWebSocket =
/*#__PURE__*/
function (_RPCWebSocket) {
  _inherits(NodeRPCWebSocket, _RPCWebSocket);

  function NodeRPCWebSocket(ws) {
    var _this;

    _classCallCheck(this, NodeRPCWebSocket);

    _this = _possibleConstructorReturn(this, _getPrototypeOf(NodeRPCWebSocket).call(this, ws));

    _this._ws.on('message', _this._reciever.onMessage);

    return _this;
  }

  _createClass(NodeRPCWebSocket, [{
    key: "ping",
    value: function ping() {
      var _this$_ws;

      (_this$_ws = this._ws).ping.apply(_this$_ws, arguments);
    }
  }, {
    key: "pong",
    value: function pong() {
      ...
      (_this$_ws2 = this._ws).pong.apply(_this$_ws2, arguments);
    }
  }, {
    key: "terminate",
    value: function terminate() {
      ...
      (_this$_ws3 = this._ws).terminate.apply(_this$_ws3, arguments);
    }
  }]);

  return NodeRPCWebSocket;
}(RPCWebSocket);
...
function (_EventEmitter) {
  ...
  function WebSocketServer(wss) {
    ...
    _defineProperty(_assertThisInitialized(_this), "__onConnection", function (ws) {
      var client = new NodeRPCWebSocket(ws);

      _this.clients.push(client);

      client.on('error', _this.__handleError);
      client.on('request', _this.__handleRequest);

      _this.emit('connection', client, _assertThisInitialized(_this));
    });

    _defineProperty(_assertThisInitialized(_this), "__handleError", function (error, client) {
      _this.emit('error', error, client, _assertThisInitialized(_this));
    });

    _defineProperty(_assertThisInitialized(_this), "__handleRequest", function (request, client) {
      _this.emit('request', request, client, _assertThisInitialized(_this));
    });

    _this._wss = wss;
    _this.clients = [];

    _this._wss.on('connection', _this.__onConnection);
...
function (_RPCWebSocket) {
  ...
  function BrowserRPCWebSocket() {
    ...
    _defineProperty(_assertThisInitialized(_this), "_onMessage", function (event) {
      _this._reciever.onMessage(event.data);
    });

    _this.onmessage = _this._onMessage;
    return _this;
  }
