var RPCWebSocket = (function () {
  ...
  var DEFAULT_REQUEST_TIMEOUT_MILLISECONDS = 5000;
  ...
  var proxyEvents = ['open', 'close', 'message'];
  ...
  function (_EventEmitter) {
    ...
    function RPCWebSocket(ws) {
      ...
      _defineProperty(_assertThisInitialized(_this), "_onWSError", function (error) {
      ...
      _this.ws.on('error', _this._onWSError);
  ...
  /*#__PURE__*/
  function (_WebSocket) {
    _inherits(BrowserWebSocket, _WebSocket);
    ...
    function BrowserWebSocket() {
      ...
      _classCallCheck(this, BrowserWebSocket);

      for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
        args[_key] = arguments[_key];
      }

      _this = _possibleConstructorReturn(this, (_getPrototypeOf2 = _getPrototypeOf(BrowserWebSocket)).call.apply(_getPrototypeOf2, [this].concat(args)));

      _defineProperty(_assertThisInitialized(_this), "on", function (event, handler) {
        if (event === 'message') {
          _this._addOnMessageEventListener(event, handler);
        } else {
          _this.addEventListener(event, handler);
        }
      });

      _defineProperty(_assertThisInitialized(_this), "remove", function (event, handler) {
        if (event === 'message') {
          _this._removeOnMessageEventListener(event);
        } else {
          _this.removeEventListener(event, handler);
        }
      });

      _defineProperty(_assertThisInitialized(_this), "_addOnMessageEventListener", function (event, handler) {
        _this._onMessageEventListener = _this._createOnMessageEventListener(handler);

        _this.addEventListener(event, _this._onMessageEventListener);
      });

      _defineProperty(_assertThisInitialized(_this), "_removeOnMessageEventListener", function (event) {
        _this.removeEventListener(event, _this._onMessageEventListener);
      });

      _defineProperty(_assertThisInitialized(_this), "_createOnMessageEventListener", function (handler) {
  ...
  function (_RPCWebSocket) {
    ...
    function BrowserRPCWebSocket() {
      ...
      return _possibleConstructorReturn(this, _getPrototypeOf(BrowserRPCWebSocket).call(this, _construct(BrowserWebSocket, args)));
    }
